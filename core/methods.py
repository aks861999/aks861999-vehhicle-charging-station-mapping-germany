...
# -----------------------------------------------------------------------------
@ht.timer
def preprop_lstat_add_charging_station_count(df_lstat2):
    """Preprocessing DataFrame from Ladesaeulenregister_SEP.xlsx and adding charging station count"""

    # Group by PLZ and count the number of charging stations
    charging_station_counts = df_lstat2.groupby('PLZ').size().reset_index(name='charging_station_count')

    # Merge the count with the original DataFrame to retain geometry and other columns
    merged_df = pd.merge(df_lstat2, charging_station_counts, on='PLZ')

    # Drop duplicates to keep only one row per PLZ with the count and geometry
    final_df = merged_df.drop_duplicates(subset=['PLZ'])

    # Select only the relevant columns
    final_df = final_df[['PLZ', 'charging_station_count', 'geometry']]
...
def make_streamlit_electric_Charging_resid(dfr1, dfr2):
    ...
    if layer_selection == "Residents":
        ...
        color_map = LinearColormap(colors=['yellow', 'red'], vmin=dframe1['charging_station_count'].min(), vmax=dframe1['charging_station_count'].max())

    # Add polygons to the map for Numbers
        for idx, row in dframe1.iterrows():
            folium.GeoJson(
                row['geometry'],
                style_function=lambda x, color=color_map(row['charging_station_count']): {
                    'fillColor': color,
                    'color': 'black',
                    'weight': 1,
                    'fillOpacity': 0.7
                },
                tooltip=f"PLZ: {row['PLZ']}, Number: {row['charging_station_count']}"
            ).add_to(m)
